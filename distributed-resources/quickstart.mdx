...
<Tabs>
  <Tab title="EV charger">
    ...
    ```json title="Registering a three-phase AC charge point"
    {
    ...
    A charge point cannot export, so `maxExportKw` is `0`. Granularity follows the current type: an AC charge point is commanded in whole amps, so it declares `Steps` — one setpoint per amp from the 6 A minimum up to 16 A across three phases, plus `0`. A DC charge point declares `Continuous` instead, such as `{ "mode": "Continuous", "minKw": -150, "maxKw": -5 }`, because its converter follows any power setpoint inside the band. Neither reaches arbitrarily small power: below the lowest step, or below `minKw`, the only reachable setpoint is `0`.
...
Do not acknowledge a discarded activation. By the time the acknowledgement would arrive, the platform has already discarded the activation on its side.

---

## Step 5 — Acknowledge activations

Acknowledge every activation you do not discard as stale, for every resource type.
